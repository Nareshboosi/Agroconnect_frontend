...
const EditFarmerCrop = () => {
  ...
  if (loading) {
    return <p className="text-center mt-20 text-gray-500">Loading crop...</p>;
  }

  return (
    <div className="min-h-screen flex items-start justify-center px-4">
      <div className="w-full max-w-lg mt-10 bg-white p-6 rounded-2xl shadow animate-fadeIn">
        <h2 className="text-2xl font-semibold text-center text-green-700 mb-6">
          🌾 Edit Crop
        </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {[
            ...
            { name: "cropType", placeholder: "Crop Type" },
            {
              name: "availableQuantity",
              placeholder: "Available Quantity (kg)",
              type: "number",
            },
            { name: "price", placeholder: "Price per kg", type: "number" },
            { name: "season", placeholder: "Season" },
          ].map((field) => (
            <input
              key={field.name}
              ...
              required={field.name !== "cropType" && field.name !== "season"}
